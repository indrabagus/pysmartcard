...
!define un.CheckForPython $CheckForPython
...
; This must be a macro because the registry root key cannot be a variable
!macro un.EnumeratePython id hk_root hk
    StrCpy $0 0
    loop${id}:
        ClearErrors
        EnumRegKey $1 ${hk_root} ${hk} $0
...
Function un.ValidatePythonVersion
  ClearErrors
  ; We should work in python 32 bit
  nsExec::ExecToStack '"$PythonExecutable" "-c" "import sys; exit ({True:0,False:1}[sys.maxsize < 2**32])"'
  Pop $1
...
Function un.CheckForPython
    ; Find all in these path, however if your system 64 bit and you installed 32 bit we still able to enumerate the HKLM\Software\Python 
    ; since it will  mapped to HKLM\Software\Wow6432Node\Python
    !insertmacro un.EnumeratePython 1 HKLM Software\Python\PythonCore
    !insertmacro un.EnumeratePython 2 HKLM Software\Wow6432Node\Python\PythonCore
    !insertmacro un.EnumeratePython 3 HKCU Software\Python\PythonCore
    !insertmacro un.EnumeratePython 4 HKCU Software\Wow6432Node\Python\PythonCore
